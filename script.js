...
function buildQuestions(settings, source = VOCAB_DATA) {
  let items = settings.order === "random" ? shuffle(source) : [...source];
  const count = settings.count === "all" ? items.length : Number(settings.count);
...
function startQuiz(source = VOCAB_DATA) {
  const settings = {
  ...
  currentQuestions = buildQuestions(settings, source);
  currentIndex = 0;
  score = 0;
  checked = false;
  wrongQuestions = [];
...
function showQuestion() {
  ...
  if (q.type === "meaning") {
    $("modeLabel").textContent = "Meaning Quiz";
    ...
    $("modeLabel").textContent = "Sentence Quiz";
...
$("wrongOnlyBtn").addEventListener("click", () => {
  ...
  const source = wrongQuestions.map(({ userAnswer, type, ...rest }) => ({ ...rest, type }));
